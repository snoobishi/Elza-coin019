...
const MANAGER_USERNAME = process.env.MANAGER_USERNAME || "Elzakarygustinvil019";
...
async function handleApi(req, res) {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/register") {
      ...
      // Give 100x autoclick to first 15 new users
      if (!db.newUserCount) db.newUserCount = 0;
      if (db.newUserCount < 15) {
        db.users[key].autoclick100 = true;
        db.users[key].autoClickLevel = 100;
        db.newUserCount++;
      }
    ...
    if (req.method === "POST" && req.url === "/api/claims") {
      ...
      if (!walletType || !walletType.match(/^(Binance|Google Pay|Moncash|NatCash)$/)) return sendJson(res, 400, { error: "Choose Binance, Google Pay, Moncash or NatCash." });
      
      // Moncash and NatCash accept phone numbers (7+ chars), Binance/Google Pay need longer addresses (8+ chars)
      const minLength = (walletType === "Moncash" || walletType === "NatCash") ? 7 : 8;
      if (walletAddress.length < minLength) return sendJson(res, 400, { error: `Enter a valid ${walletType} ${(walletType === "Moncash" || walletType === "NatCash") ? "phone number" : "address"}.` });
    ...
    if (req.method === "POST" && req.url === "/api/upgrade") {
      ...
      const upgrades = [
        ...
        { id: "autoclick100", cost: 0, type: "autoclick", level: 100, newUsersOnly: true }
      ];
    ...
    if (req.method === "POST" && req.url === "/api/admin/withdraw") {
      ...
      if (!withdrawType || !withdrawType.match(/^(Binance|Google Pay|Moncash|NatCash)$/)) {
        return sendJson(res, 400, { error: "Choose Binance, Google Pay, Moncash, or NatCash." });
      }

      // Moncash and NatCash accept phone numbers, others need longer addresses
      const minLength = (withdrawType === "Moncash" || withdrawType === "NatCash") ? 7 : 8;
      if (walletAddress.length < minLength) {
        return sendJson(res, 400, { error: `Enter a valid ${withdrawType} ${(withdrawType === "Moncash" || withdrawType === "NatCash") ? "phone number" : "address"}.` });
      }
...
server.listen(PORT, () => {
  console.log(`Elza-coin019 running at http://localhost:${PORT}`);
});
